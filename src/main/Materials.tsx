...
import { Search, Settings2, Trash, BookOpen, X, Save, Camera, Plus, ArrowUp, ArrowDown, ChevronsUpDown, Minus, FileDown, Palette, Ruler, Tag, Calendar, TrendingUp, TrendingDown, BarChart3, Bell, BellRing, CheckCircle, AlertTriangle, Info, XCircle } from 'lucide-react';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { collection, query, orderBy, getDocs, doc, deleteDoc, addDoc, updateDoc, where, Timestamp } from 'firebase/firestore';
...
interface Notification {
  ...
  material_name: string;
  message: string;
  type: 'low_stock' | 'out_of_stock' | 'restocked' | 'deduction';
  is_read: boolean;
  created_at: string;
  stocks?: number;
  threshold?: number;
...
export default function Materials() {
  ...
  const [notificationSocket, setNotificationSocket] = useState<EventSource | null>(null);
  ...
  const markAllAsRead = async () => {
    try {
      ...
      for (const notification of unreadNotifications) {
        await updateDoc(doc(db, 'notifications', notification.id), { is_read: true });
      }
      await fetchNotifications();
  ...
  const checkStockLevels = async (material: Material, oldStock?: number) => {
    const currentStock = material.stocks;
    const threshold = material.low_stock_threshold || 5;
    ...
    if (currentStock === 0) {
      await createNotification(
        material.id,
        material.name,
        `${material.name} is now OUT OF STOCK! Please restock immediately.`,
        'out_of_stock',
        currentStock,
        threshold
      );
    ...
    // Check for low stock
      ...
      await createNotification(
        material.id,
        material.name,
        `${material.name} is running low (${currentStock} ${material.unit} remaining). Threshold: ${threshold} ${material.unit}. Please restock soon.`,
        'low_stock',
        currentStock,
        threshold
      );
    ...
    // Check if restocked (old stock was 0 or low and new stock is above threshold)
    if (oldStock !== undefined) {
      if (oldStock <= threshold && currentStock > threshold) {
        await createNotification(
          material.id,
          material.name,
          `${material.name} has been restocked! Now has ${currentStock} ${material.unit} available.`,
          'restocked',
          currentStock,
          threshold
        );
  ...
  const fetchMaterials = async () => {
    try {
      ...
      // Check stock levels for all materials on load
      for (const material of data) {
        await checkStockLevels(material);
      }
    } catch (error) {
    ...
    try {
      ...
      if (user && materialToDelete) {
        ...
        await createNotification(
          ...
          `${materialToDelete.name} has been removed from inventory.`,
          'deduction',
          materialToDelete.stocks
        );
      }
  ...
  const handleDeduct = async () => {
    ...
    try {
      ...
      const updatedMaterial = { ...selectedDeductItem, stocks: newStock };
      await checkStockLevels(updatedMaterial, oldStock);
      
      await createNotification(
        selectedDeductItem.id,
        selectedDeductItem.name,
        `${qty} ${selectedDeductItem.unit} of ${selectedDeductItem.name} was deducted. Reason: ${deductReason || 'No reason provided'}. Remaining stock: ${newStock} ${selectedDeductItem.unit}.`,
        'deduction',
        newStock
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === 'edit') {
        ...
        const updatedMaterial = { ...oldMaterial, ...materialData, id: formData.id } as Material;
        await checkStockLevels(updatedMaterial, oldMaterial?.stocks);
        ...
        // Check stock levels for new material
        await checkStockLevels({ ...materialData, id: docRef.id } as Material);
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
          ...
          <div className="relative">
            <button
              ...
              className="relative flex items-center gap-2 text-sm font-semibold cursor-pointer text-gray-700 bg-white border border-gray-200 px-3 py-1.5 rounded-md hover:bg-gray-50 transition-all"
            >
              {unreadCount > 0 ? (
                <>
